...
class Account:
    ...
    def login(self):
        ...
        if self.username == '' or self.password == '':
            ...
            xbmc.log('THROMER requests.post %s' % url, level=xbmc.LOGINFO)                                                                             
            r = requests.post(url, headers=headers, data=payload, verify=self.verify)
            xbmc.log('THROMER requests.posted %s' % url, level=xbmc.LOGINFO)                                                                             
            if r.ok:
    ...
    def get_stream(self, content_id):
        stream_url, headers_map, broadcast_start = self.get_stream_with_more_headers_as_map(content_id)
        entries = []
        for k, v in headers_map.items():
            entries.append('%s=%s' % (k, v))
        return stream_url, '&'.join(entries), broadcast_start
